...
typedef struct __myarg_t { //Thread argumanlari.a ve b den aldigim ve carpacagim sayilar.(Slayttaki gibi olusturdum.)
 int a;
 int b;
} myarg_t;

typedef struct __myret_t { //Thread carpim sonucunu basacagim sayi.(Slayttaki gibi olusturdum.)
int c;
} myret_t;

void* mythread(void *arg) { //Bu ise Threadim. a ile b yi carpiyor ardindan c ye yaziyor.
  myarg_t *m = (myarg_t *) arg;
  myret_t *r = malloc(sizeof(myret_t)); //Allocated in heap.Sunumda stackte olusturmak tehlikeli diyordu!!!Kaybolabilirmis.
  r->c = m->a * m->b;
  return (void *)r;
}
...
int* matrisBoyutBul(char dosya[]){ //Bu fonksiyondaki amacim olusturacagim matrisin boyutunu bulmak.
  ...
  while (fgets(satirBilgi, 20, d) != NULL) { //Bu kismi internetten aldim.
    satirSayisi++; //Her satirda bir atar.Boylece satir sayimi bulurum.
  }
  fclose(d);
  //NOT:Eger sonuc dogru degilse alt +1 i silin/ekleyin.Cunku dosyalardan matrisi okurken satir sonundaki bosluk kodu etkiliyor.
...
int** matrisDoldur(char dosya[],int satir,int sutun,int **matris){ //Bu fonksiyonda amacim olusturdugum matrise dosyadaki degerleri yazmak.
  ...
  for (int i = 0; i < satir; i++) {
    for (int j = 0; j < sutun; j++) {
      int sayi;
      fscanf(d, "%d", &sayi); //Bu kod parcasini internetten aldim.Verilen dosya akısindan karakter okuyor ve okuduğu karakterleri sayi degiskenine atıyorum.
    ...
    for (int j = 0; j < sutunSayisiB; j++) {
      ...
      for (int k = 0; k < satirSayisiB; k++) {
        //matrisc[i][j] += matrisa[i][k] * matrisb[k][j]; normal matris carpimi yapsaydım bu kodu kullanacktım.Biz çarpimi threadin icinde yaptik.Sonra toplama yaparak yeni oluşturdugum matrise yazdim.
      	...
      	//Sunumdaki gibi thread olusturdum.
      	int rc;
      	pthread_t p;
        myarg_t args;
